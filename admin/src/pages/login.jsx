    ...
    try {
      if (state === "admin") {
        ...
        const { data } = await axios.post(
          backend_url + "/api/doctor/doctor-login",
          { email, password },
        );
        if (data.token) {
          console.log(data);
      ...
      >
        <div className="flex flex-col gap-6 m-auto items-start p-8 min-w-[340px] sm:min-w-96 rounded-xl text-sm text-gray-600 shadow-lg">
          <p className="flex items-center gap-2 text-2xl font-semibold capitalize mx-auto">
            <span className="text-main">{state}</span>
            <span>login</span>
          </p>
          <div className="flex flex-col gap-2">
            ...
            <input
              ...
              type="email"
              required
              className="w-[320px] px-4 py-2 border-1 border-gray-300 rounded-md outline-0"
            />
          </div>
          <div className="flex flex-col gap-2">
            <p className="text-lg font-medium text-gray-500 capitalize">
          ...
          <button className="bg-main text-white rounded-lg px-4 py-2 cursor-pointer w-full text-lg">
            Login
          </button>
          {state === "admin" ? (
            <p className="capitalize flex items-center gap-2 mx-auto">
