...
export default function DoctorDashboard() {
  const { dashData, getDashData, dToken, cancelAppointment, markCompleted } =
  ...
  return (
    <>
      <div className="flex flex-col gap-4 px-4 py-4">
        <p className="font-semibold mt-3 ml-4 text-lg capitalize">
          doctor dashBoard
        </p>
        {dashData && (
          <div className="flex flex-col gap-8">
            ...
            <div className="flex flex-col gap-8">
              <div className="flex items-center gap-3 p-4 rouded-md mt-8 text-xl font-semibold capitalize">
                <FaRegListAlt className="text-main" />
                <p>latest booking</p>
              </div>
              ...
              <div className="hidden sm:block">
                {dashData.latestAppointments.map((item, ind) => (
                  <div
                    className="items-center gap-4 px-6 py-2 hover:bg-gray-100 cursor-pointer grid grid-cols-[3fr_5fr_2fr] rounded-md"
                    key={ind}
                  >
                    <img
                    ...
                    {item.isCompleted ? (
                      ...
                      <div className="flex gap-2">
                        <button
                          ...
                          className="w-6 h-6 flex items-center justify-center rounded-full cursor-pointer bg-red-100 text-red-300 font-bold border border-red-300"
                        >
                          <IoClose />
                        </button>
                        <button
                          onClick={() => markCompleted(item._id)}
                          className="w-6 h-6 flex items-center justify-center rounded-full cursor-pointer bg-green-100 text-green-300 font-bold border border-green-300"
                        >
                          <IoMdCheckmark />
                        </button>
                      </div>
              ...
              <div className="block sm:hidden">
                {dashData.latestAppointments.map((item, ind) => (
                  <div
                    className="flex flex-col space-y-4 px-4 py-2 bg-gray-100 rounded-md"
                    key={ind}
                  >
                    <div className="flex gap-2">
                      <img
